...
public class SignupServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        switch (action) {
            case "/signup":
                ...
                break;
            default:
                resp.sendRedirect("signup.jsp?error=Invalid action");
        }
    }
    ...
    private void saveUser(HttpServletRequest req, HttpServletResponse resp) throws IOException, ClassNotFoundException {
        ...
        if (isSaved) {
            resp.sendRedirect("login.jsp?message=User Saved Successfully");
        } else {
            resp.sendRedirect("signup.jsp?error=Failed to save the user");
        }
    }
}
